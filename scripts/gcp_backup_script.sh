...
upload_to_gcp_bucket() {
    ...
    fqdn=$(hostname)
    if /home/master/google-cloud-sdk/bin/gsutil cp "$backup_file" gs://<enter-bucket-name>/$fqdn/$app/backup-${converted_date}.tgz; then
        echo "$(date "+%b %d %H:%M:%S") Backup file ${backup_file} uploaded successfully." >> $dir/logs/custom_offsite_backups.log
    else 
        echo "$(date "+%b %d %H:%M:%S") ploading backup file ${backup_file} failed."
    fi
}
