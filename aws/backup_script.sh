#!/bin/bash
dir="/home/master/aws_backups"

# Function to upload backup to AWS bucket
upload_to_s3_bucket() {
    app="$1"
    backup_file="$2"
    backup_date="$3"
    ...
    fqdn=$(hostname)
    if /home/master/.local/bin/aws s3 cp "$backup_file" s3://cloudways-custom-server-backups/$fqdn/$app/backup-${converted_date}.tgz; then
        echo "$(date "+%b %d %H:%M:%S") Backup file ${backup_file} uploaded successfully." >> $dir/logs/custom_offsite_backups.log
    else 
        echo "$(date "+%b %d %H:%M:%S") Uploading backup file ${backup_file} failed."
    fi
}
...
compare_and_update_latest_backup() {
    ...
    if [ "$modification_time" -gt "$latest_offsite_backup" ]; then
        ...
        upload_to_s3_bucket "$app" "$backup_file" "$mod_time"
